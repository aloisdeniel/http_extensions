...
class HttpDate {
  ...
  static DateTime parse(String date) {
    ...
    void expect(String s) {
      if (date.length - index < s.length) {
        throw new Exception('Invalid HTTP date $date');
      }
    ...
    int expectWeekday() {
      ...
      if (pos == -1) {
        int pos = date.indexOf(' ', index);
        if (pos == -1) throw new Exception('Invalid HTTP date $date');
    ...
    int expectMonth(String separator) {
      ...
      if (pos - index != 3) throw new Exception('Invalid HTTP date $date');
      tmp = date.substring(index, pos);
      index = pos + 1;
  ...
  static DateTime _parseCookieDate(String date) {
    ...
    void error() {
      throw new Exception('Invalid cookie date $date');
    }
